...
class Monitor():
    ...
    def update_gpu_values(self, status):
        self._diag_gpu.name = status.name
        self._diag_gpu.message = status.message
        self._diag_gpu.hardware_id = status.hardware_id
        # self._diag_gpu.status.pci_device_id = status.values[1].value
        self._diag_gpu.status.product_name = status.values[0].value
        # self._diag_gpu.status.pci_device_id = status.values[1].value
        # self._diag_gpu.status.pci_location = status.values[2].value
        # self._diag_gpu.status.display = status.values[3].value
        self._diag_gpu.status.driver_version = status.values[1].value
        self._diag_gpu.status.temperature = status.values[2].value
        self._diag_gpu.status.fan_speed = status.values[3].value
        self._diag_gpu.status.gpu_usage = status.values[4].value
        self._diag_gpu.status.memory_usage = status.values[5].value
...
def callback(data):
    ...
    elif len(data.status) > 1 and data.status[1].name.startswith('CPU Usage'):
    ...
    elif data.status[0].name.startswith("GPU Status"):
        # Extract useful data from disk
        mon.update_gpu_values(data.status[0])
    mon.publish_info()
